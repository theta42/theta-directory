...
- fix: **User and group mutations on the master now trigger a live resync push to spokes.** LDAP identity changes rely on OpenLDAP syncrepl over the WireGuard mesh, but when the mesh tunnel isn't carrying packets those changes never reached spokes. `routes/user.js` and `routes/group.js` now fire the same fire-and-forget resync ping that directory resources already used, so spokes re-import the master's full LDAP snapshot and stay in sync over the public HTTPS fallback.
- fix: **Personal access tokens (`sso_...`) are now part of the replicated catalog.** `ApiToken` moved from the Redis-backed `model-redis` store into the replicated SQLite catalog. The master export includes the bcrypt hashes, and the spoke import adopts them, so a token minted on the master is valid on every spoke. The raw token secret is still only shown once at creation time and is never stored or replicated.
- refactor: extracted `utils/replicate_on_finish.js` so user, group, and directory routes share one implementation of the post-write resync trigger.
