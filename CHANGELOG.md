...
### Fixed
- **No Agent Has Received A `config` Frame Since v2.36.14**: the frame is assembled in an async IIFE, and v2.36.14 added `const orgName = (conf.name && …)` to it — in a closure where `conf` is not in scope (it was required locally at two *other* points in the file, never at module level). The ReferenceError threw before `ws.send`, was swallowed by the enclosing `catch`, and left one "Failed to send config payload" line per connection. Everything on that frame was lost for every agent in the fleet: `agent_id`, `site_lan_endpoint`, `site_public_ip`, `site_name`, `organization_name`, and — fatally — the `auth_token` and `public_key` a join-key host must persist. A fresh install therefore enrolled once, learned nothing, re-dialled with its join key, collided with its own hostname, and was locked out `4001` permanently by the G-2 check. `conf` is now required at module scope, and the optional half of the payload is gathered piece by piece so nothing in it can ever cost an agent its credentials again.
- **`ThetaAgentDriver` Could Not Dispatch A Single Command**: all four call sites passed `agent.id` where `AgentManager.sendCommand(agent, …)` expects the row — it resolves the socket via `agent.id`, so a string made it look up `undefined` and throw `Agent "undefined" is not connected`. Every reboot, service start/stop/restart, desktop action and scrub from the directory failed before reaching the wire. Two of those sites also passed `isHighRisk` as a *payload key* rather than the signing argument, which signs nothing: the flag rode along inside the command while the frame went out unsigned, and the agent refuses every `systemd_action` but `status` without a signature. The driver's unit test mocked `sendCommand` and asserted the payload-key form, so it was green throughout; it now asserts the real contract.
- **`HIGH_RISK_COMMANDS` Was Out Of Sync With The Agent's Signature Gates**: `desktop_control` (and its four aliases), `wireguard_apply`, `wireguard_remove`, `register_service`, `unregister_service` and `zpool_scrub` are all verified by the agent and none were signed from this list, so any caller of `POST /nodes/:id/command` for them got "signature verification failed" — which reads as a key problem and is not one. The mesh and driver paths were unaffected (they pass `isHighRisk` explicitly).
- **The Agent's MAC Address Was Dropped On Arrival**: `handleDiscovery` builds an explicit whitelist of discovery fields and `mac_address` was not in it, so every read of `discovery.mac_address` in `applyDiscoveryToDirectory` saw `undefined`: the MAC tier of host adoption never fired, an unbound agent's host slug always fell back to its hostname instead of `host-<mac>`, and the host row never recorded a MAC for any other discovery source to match against. The adoption tests pass `applyDiscoveryToDirectory` a MAC directly, which is exactly why none of them could see it.
- **Nothing Detected A Dead Agent Socket**: `ws` never pings on its own and nothing here did either, so a half-open socket (NAT timeout, a host that lost power, a dropped tunnel) kept `readyState` OPEN indefinitely — `isConnected()` stayed true, the fleet view showed the agent up, and `dispatchCommandClusterWide` wrote commands into a socket nobody was reading instead of forwarding them to the node that really held the agent. A 30s ping with termination on a missed pong now mirrors the check the agent already performs in the other direction.
- **Command Responses From Released Agents Were Dropped**: agents through v2.21.9 answer with a bare `{status, message, …}` and no envelope; this handler returns early on any frame without a string `type`, so `lastResponse` was permanently null. Fixed properly in theta-agent v2.22.0, and a typeless frame carrying a `status` is now read as the response it is, so an un-upgraded fleet works too.
- **A Service Removed From `agent.yml` Kept Its Resource Forever**: PROTOCOL.md §3.5 has promised since it was written that the directory drops the child resource "on the next reconciliation"; `reconcileServicesFromTelemetry` only ever created and updated. A name deleted by hand — or a host whose `unregister_service` frame was lost — left a child resource reporting stale health indefinitely. Telemetry now prunes what the agent no longer reports, scoped to what this source created: a service another source also sees is kept and merely loses `theta-agent` from its `discovery_sources`, because this agent no longer watching something is not evidence the thing is gone. An *absent* `services` key still prunes nothing (an older agent omitted it when empty); only an explicit empty list means "none left".
- **`zfs_pool` Resources Reported A Hardcoded "ONLINE"**: the driver read `telemetry.zfs` and `telemetry.wireguard`, with a default standing in for them, and neither key has ever existed on the wire — so every ZFS pool in the catalog reported healthy whatever it was really doing, and every WireGuard resource an empty peer list. Both now read the fields the agent actually sends (`zfs_health`, and the new `wireguard` block in theta-agent v2.22.0) and report `null` plus a `reported` flag when there is no reading, because a default indistinguishable from a healthy answer is worse than no answer.
- **Two Disagreeing Mesh Push Paths**: `POST /api/mesh/clients/:id/push` built its own `wireguard_apply` carrying the config alone — no `siteId`/`exitSiteId` — while enrolment and exit selection push the same config *with* the fields that tell the agent whether to raise the tunnel (PROTOCOL.md §4.3.1). It now calls the same `meshClients.pushConfigToAgent` as the other two, and reports 409 when the agent is not connected rather than appearing to succeed.

### Added
- **Node IAM Has A Caller (`GET`/`POST /api/agent/nodes/:id/iam`)**: `iam_apply` sat in `HIGH_RISK_COMMANDS` with nothing in the directory ever sending it, so the agent's entire IAM engine was unreachable and a host's login policy was whatever its own `/etc` happened to say. `utils/agent_iam.js` derives `allowed_login_groups` from the group model (`docs/GROUPS.md`) including grants inherited from ancestor resources, excluding meta groups and anything below `access` — catalog visibility is not a shell. `GET` returns exactly what would be sent (and whether the agent's `iam` capability is even on); `POST` signs and sends it. Deliberately operator-triggered rather than pushed on connect, unlike `configure_ldap`: the agent writes a deny-all `/etc/security/access.conf`, and a reconnect is no reason to change who can log into a fleet. `sudo_rules`, `ssh_keys` and `revoke_users` are deliberately left empty — the only sudo rule derivable from "admin on this host" is `ALL/ALL`, which is the landmine H12 removed from the LDAP side (scoped sudo is design gap D5); SSH keys are served by the agent's `AuthorizedKeysCommand`; and a revocation list computed at push time names people who are already gone.
- **The Agent WebSocket Handler Has Tests**: `initAgentWebSockets` — authentication, join-key enrollment, the config frame, the message switch — had none, which is how both regressions above shipped and stayed invisible. `tests/agent_ws_handler.test.js` covers the credentials on the config frame, contract G-2 in both directions (a join key cannot take over a hostname it cannot prove it owns; the real host presenting its superseded token is rotated onto a fresh one), the legacy typeless response, and revocation taking effect mid-session.

### Changed
- **The Served `install.sh` Is No Longer A Stale Fork**: the copy under `public/resources/theta-agent/` had drifted to 283 lines against the theta-agent submodule's 721 — no WireGuard tools install (so an agent enrolled into the mesh, was allocated an address, got a peer at the gateway and its pushed config, then failed at `wg-quick up`), no stray-agent cleanup, no tray install. Anyone installing from the Install Agent page got that installer. Re-synced from the submodule, which `setup.sh` now stages directly.
...
### Fixed
- **Resource Identity Matcher Safety Gap**: `utils/agent_manager.js`'s host-consolidation logic (merging an agent's auto-created placeholder host into a richer existing one) maintained its own independent MAC/IP/name matcher with no site scoping — despite a comment claiming one — and no guard against hijacking a resource that already has its own MAC identity. A false match there deletes the placeholder host and reparents its edges, so this was the highest-severity gap of the four. Extracted `services/discovery_reconciler.js`'s well-guarded 4-tier matcher (UUID → MAC → IP → name/slug, with the MAC-hijack guard and same-site scoping) into a new standalone module, `services/resource_matcher.js`, and pointed `agent_manager.js` at it instead of its private copy.
- **theta-agent Telemetry Never Outranked Proxmox for Guest Hosts**: `services/driver_registry.js`'s documented tier-1 rule ("prefer a connected theta-agent over a hypervisor API") never actually fired for a Proxmox-guest host with a bound agent, because `AgentManager.getAgentForResource()` had no "host → its own agent-service child" resolution direction — only "resource IS the agent" and "resource's *parent* has the agent." This is also why a fully-built richer telemetry dashboard (CPU model, kernel, memory breakdown) and an already-working active-sessions feature never rendered for any Proxmox guest with an agent installed. Added the missing direction.
- **Status Badge Contradicted Its Own State Text**: A service with no reported telemetry at all (`svc.active === undefined`) rendered a green "Active" badge in `views/directory.ejs` directly beside a "State: inactive" row for the same value — `svc.active !== false` treated the absence of data as confirmed-active in one expression and confirmed-inactive in a sibling expression. Introduced a real `active`/`inactive`/`unknown` tri-state (`public/js/resource_status.js`); an unknown state now disables every control button (Start, Stop, Restart, Reload) rather than assuming the service is running.
- **Divergent Status-Bucket Counting**: Three independent copies of the same health-bucket counting logic in `views/directory.ejs` disagreed on whether an unevaluated resource with a known `powerState` counted as healthy/warning or fell straight to unknown. Consolidated into one shared function.

### Added
- **Per-Source Fact Provenance**: Every discovery source's contribution to a resource's metadata now survives being merged, additively, under `metadata.facts_by_source` — alongside the existing flat metadata merge, which is completely unchanged. See `docs/resource-facts.md` for the canonical fact vocabulary, now a hard requirement for plugin/driver authors who want their numbers findable across sources.
- **Meshed Facts & Controls Card**: A host's own facts (CPU, memory, disk, network identity) now mesh with its children's on the Status tab — one badge per contributing source/resource, bordered when two sources disagree by more than 10%, with a hover tooltip showing how fresh each value is — and a controllable child's Start/Stop/Restart/Reload buttons are pulled into the same card. Replaces the old "Child Workloads & Status Rollup" table on the General tab, which only restated the same children already listed under Placement without integrating anything about them.
