 ...
 *
 * Called from socket_pubsub.liveBus — the single point every gated event flows
 * through, ORM-managed or not — so nothing needs to remember to log.
 *
 * Best-effort by design: a model write must never fail because its history row
 * did not save.
 *
 * NOTE: ActivityEvent must never appear in socket_pubsub's READERS. LIVE_MODELS
 * is derived from READERS, so staying out of it keeps ActivityEvent off the bus
 * — and therefore out of this function, which would otherwise record its own
 * writes forever. There is a test pinning that.
 */
